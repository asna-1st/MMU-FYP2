<script>
	...
	const formData = {
		...
		country: $modalStore[0].meta.country
	};
	...
	function validateForm() {
		...
		if (!formData.email.trim()) {
			errors.description = 'Email is required';
		}
...
{#if $modalStore[0]}
	<div class="modal-example-form {cBase}">
		...
		<form class="modal-form {cForm}">
			<label class="label">
				<span>Name</span>
				<input class="input" type="text" bind:value={formData.userName} required />
				{#if errors.userName}<p class="error">{errors.userName}</p>{/if}
			</label>
			<label class="label">
				<span>Email</span>
				<input class="input" type="email" bind:value={formData.email} required />
