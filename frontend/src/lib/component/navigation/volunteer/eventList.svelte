...
<script>
  ...
  <main class="container mx-auto mt-8 p-4">
	...
	<div class="infinite-scroll-container" style="height: 100vh; overflow-y: auto;" on:scroll={handleScroll}>
	  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
		{#each events as event (event.id)}
		  <!-- svelte-ignore a11y-click-events-have-key-events -->
		  <!-- svelte-ignore a11y-no-static-element-interactions -->
		  <div on:click={() => {goto('/chat/testUI/testEventJob')}} class="bg-white card card-hover overflow-hidden shadow-md variant-filled">
			<div class="p-4">
			  <h2 class="text-2xl font-semibold mb-2">{event.title}</h2>
			  <p class="">Start Date: {event.date}</p>
			  <p class="">End Date: {event.date}</p>
			  <p class="">Organization: {event.Organizer}</p>
			</div>
		  </div>
		{/each}
